...
public class QuestionController {
     ...
     * AI 生成题目
     *
     * @param questionAIGenerateRequest 题目类型 例如 java
     * @param request                   请求对象，用于获取登录用户信息
     * @return 是否生成成功
     */
    @PostMapping("/ai/generate/question")
    @AuthCheck(mustRole = UserConstant.ADMIN_ROLE)
    public BaseResponse<Boolean> aiGenerateQuestion(@RequestBody QuestionAIGenerateRequest questionAIGenerateRequest,
                                                    HttpServletRequest request) {
        ...
        ThrowUtils.throwIf(number <= 0 || number > 10, ErrorCode.PARAMS_ERROR, "题目数量不能小于0或者大于10");
        // 获取当前登录的管理员用户
        User loginUser = userService.getLoginUser(request);
        // 调用service层方法生成题目
        boolean result = questionService.aiGenerateQuestion(questionType, number, loginUser);
        ThrowUtils.throwIf(!result, ErrorCode.OPERATION_ERROR, "AI生成题目失败");
        return ResultUtils.success(result);
    }
